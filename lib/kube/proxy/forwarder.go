...
func (f *Forwarder) exec(ctx *authContext, w http.ResponseWriter, req *http.Request, p httprouter.Params) (interface{}, error) {
	...
	if err := setupHeaders(ctx, sess, req); err != nil {
		return nil, trace.Wrap(err)
	}
...
// wrapTransport returns a new http.RoundTripper which adds authentication,
// impersonation and forwarding headers before passing a request to rt.
func (f *Forwarder) wrapTransport(rt http.RoundTripper, ctx *authContext, sess *clusterSession) (http.RoundTripper, error) {
	// Add kubeconfig credentials.
	rt, err := f.creds.wrapTransport(rt)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	// Add impersonation and forwarding headers.
	return wrapSetupHeaders(rt, ctx, sess), nil
}
